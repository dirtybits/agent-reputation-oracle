...
async function main() {
  ...
  if (!voucherKeypairPath || !voucheeAddress) {
    console.error(
      "Usage: ts-node vouch.ts <voucher-keypair-path> <vouchee-address> [amount-sol]"
    );
    process.exit(1);
  }
  ...
  try {
    const tx = await program.methods.vouch(stakeAmount).rpc();
    ...
    console.log(
      "  Total Staked For:",
      (
        profile.totalStakedFor.toNumber() / anchor.web3.LAMPORTS_PER_SOL
      ).toFixed(4),
      "SOL"
    );
  } catch (e: any) {
