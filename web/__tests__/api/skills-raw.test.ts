...
import { generatePaymentRequirement, verifyPaymentProof, settlePayment } from '@/lib/x402';
...
const mockSettle = settlePayment as unknown as ReturnType<typeof vi.fn>;
...
describe('GET /api/skills/[id]/raw', () => {
  ...
  it('returns 402 with payment requirement for paid skill without proof', async () => {
    const dbQuery = vi.fn().mockResolvedValueOnce([
    ...
    mockGenerate.mockReturnValue({ scheme: 'exact', network: 'solana', amount: 100_000_000 });
  ...
  it('returns content when valid payment proof is provided', async () => {
    ...
    mockSettle.mockResolvedValue({ settlementId: 'ref-1', status: 'complete' });

    const proof = JSON.stringify({ txSignature: 'tx123', requirement: {} });
  ...
  it('returns 402 when payment proof verification fails', async () => {
    ...
    mockVerify.mockResolvedValue({ status: 'invalid', paymentRef: 'ref-2', error: 'TX failed' });

    const proof = JSON.stringify({ txSignature: 'badtx', requirement: {} });
