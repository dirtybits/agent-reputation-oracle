...
describe("GET /api/skills/[id]/raw", () => {
  ...
  it("returns 400 when x-payment-proof header is malformed", async () => {
    ...
    const { req, params } = makeRequest("uuid-6", {
      "x-payment-proof": "not-json!!!",
    });
    const res = await GET(req, { params });
