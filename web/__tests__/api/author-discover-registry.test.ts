...
function makeRequest(pubkey: string, body: Record<string, any> = {}) {
  const req = new NextRequest(`http://localhost/api/author/${pubkey}/discover-registry`, {
    method: 'POST',
...
describe('POST /api/author/[pubkey]/discover-registry', () => {
  ...
  it('returns 401 when signature is invalid', async () => {
    ...
    const { req, params } = makeRequest('Author111', { auth: { pubkey: 'Author111' } });
  ...
  it('returns 403 when the viewer wallet does not match the author wallet', async () => {
    ...
    const { req, params } = makeRequest('Author111', { auth: { pubkey: 'OtherWallet' } });
    const res = await POST(req, { params });
