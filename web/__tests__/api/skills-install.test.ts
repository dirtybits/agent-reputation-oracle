...
describe('POST /api/skills/[id]/install', () => {
  ...
  it('returns 401 when signature is invalid', async () => {
    mockVerify.mockReturnValue({ valid: false, pubkey: null, error: 'Invalid signature' });
    const { req, params } = makeRequest('some-id', { auth: { pubkey: 'x', signature: 'y', message: 'z', timestamp: Date.now() } });
    const res = await POST(req, { params });
  ...
  it('returns 200 for free chain-prefixed skill', async () => {
    ...
    const { req, params } = makeRequest('chain-ABC123', { auth: { pubkey: 'Wallet1' } });
    const res = await POST(req, { params });
  ...
  it('returns 402 for paid chain-prefixed skill', async () => {
    ...
    const { req, params } = makeRequest('chain-DEF456', { auth: { pubkey: 'Wallet1' } });
    const res = await POST(req, { params });
  ...
  it('returns 404 for missing chain-prefixed skill', async () => {
    ...
    const { req, params } = makeRequest('chain-NOPE', { auth: { pubkey: 'Wallet1' } });
    const res = await POST(req, { params });
  ...
  it('returns 200 for free repo skill with no on_chain_address', async () => {
    ...
    const { req, params } = makeRequest('uuid-1', { auth: { pubkey: 'Wallet1' } });
    const res = await POST(req, { params });
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it('returns 402 for repo skill with paid on-chain listing', async () => {
    ...
    const dbQuery = vi.fn().mockResolvedValueOnce([{ id: 'uuid-2', on_chain_address: 'ChainAddr' }]);
    mockSql.mockReturnValue(dbQuery);

    const { req, params } = makeRequest('uuid-2', { auth: { pubkey: 'Wallet1' } });
    const res = await POST(req, { params });
    expect(res.status).toBe(402);
  });
  ...
  it('returns 404 when repo skill not found', async () => {
    ...
    const { req, params } = makeRequest('uuid-missing', { auth: { pubkey: 'Wallet1' } });
    const res = await POST(req, { params });
    expect(res.status).toBe(404);
  });
});
